/**
 * Sprint 8 Wave 2 scenario seeds — NP-003, NP-004, NP-005, NP-006, NP-007
 *
 * Usage:
 *   node scripts/seed-s8-wave2-scenarios.mjs --scenario=NP-003
 */
...
async function createProjectBase(npId, title, designBasisStatus, includeTor, scopeDescription) {
  ...
  await prisma.design_basis_versions.create({
    data: {
      ...
      approval_status: designBasisStatus,
    },
  });

  const docTypes = includeTor
    ? [
...
async function seedNp003() {
  return {
    ...(await createProjectBase(
      "NP-003",
      "NP-003 Export While BLOCK Exists Project",
      "Draft",
      true,
      "Power distribution scope — export gate drill.",
    )),
...
async function seedNp004() {
  return {
    ...(await createProjectBase(
      "NP-004",
      "NP-004 Handoff Without Target Project",
      "Approved",
      true,
      "Power distribution scope — handoff target drill.",
    )),
    seed_profile: "handoff-without-target",
  };
}

async function seedNp005() {
  return {
    ...(await createProjectBase(
      "NP-005",
      "NP-005 Re-open Approved BOQ Project",
      "Approved",
      true,
      "Power distribution scope — re-open drill.",
    )),
    seed_profile: "reopen-approved-boq",
  };
}

async function seedNp006() {
  return {
    ...(await createProjectBase(
      "NP-006",
      "NP-006 Export After Approval Revoked Project",
      "Approved",
      true,
      "Power distribution scope — revoke export drill.",
    )),
    seed_profile: "export-after-revoke",
  };
}

async function seedNp007() {
  return {
    ...(await createProjectBase(
      "NP-007",
      "NP-007 Warning + Block Coexistence Project",
      "Draft",
      true,
      null,
    )),
...
async function main() {
  ...
  const scenario = (args.scenario ?? "NP-003").replace(/^seed-/, "").toUpperCase();
  if (!SEEDERS[scenario]) {
    throw new Error(`Unsupported scenario ${scenario}. Use NP-003..NP-007.`);
  }
