/**
 * Official Runner — SIM-003 Blocked Path (Sprint 7B Phase 3A)
 *
 * Preconditions (must be satisfied BEFORE running):
 *   - Phase 3 Blocked Path Plan approved
 *   - E0 baseline PASS (typecheck + test)
 *   - Fresh DB seed via scripts/seed-sprint-7b-scenarios.mjs --scenario=SIM-003
 *
 * Output (official evidence):
 *   - docs/SPRINT_7B/evidence/SIM-003/E1..E9
 *   - docs/SPRINT_7B/EXECUTION_REPORT/SIM-003.md
 *
 * Stop-on-fail: approval/handoff/export success with unresolved BLOCK = immediate FAIL.
 *
 * Usage:
 *   npx tsx scripts/execute-sim-003-official.mjs --project=<projectId> --boq=<boqVersionId>
 */
...
const EVIDENCE_DIR = path.resolve("docs/SPRINT_7B/evidence/SIM-003");
const EXPORT_DIR = path.join(EVIDENCE_DIR, "E7-export-result");
const REPORT_PATH = path.resolve("docs/SPRINT_7B/EXECUTION_REPORT/SIM-003.md");

const EXPECTED_BLOCK_RULES = ["DESIGN_BASIS_NOT_APPROVED", "DOC_TOR_REQUIRED"];
const ACCEPTABLE_APPROVAL_BLOCK_CODES = [
...
async function main() {
  ...
  if (!projectId || !boqVersionId) {
    throw new Error(
      "Usage: npx tsx scripts/execute-sim-003-official.mjs --project=<id> --boq=<id>",
    );
  }
  ...
  if (seed.project.project_name !== "SIM-003 Blocked Path Project") {
    throw new Error(
      `Expected SIM-003 project name; got ${seed.project.project_name}`,
    );
  }
  if (seed.designBasis.some((d) => d.approval_status === "Approved")) {
    throw new Error("SIM-003 seed must not have Approved design basis");
  }
  if (seed.documents.some((d) => d.document_type === "TOR")) {
    throw new Error("SIM-003 seed must omit TOR document");
  }
  ...
  const handoffAttempt = await expectBlocked(
    ...
    () =>
      handoffService.createHandoff(
        boqVersionId,
        "director-001@sim003",
        "SIM-003 Blocked Path handoff attempt (must fail)",
        "ClientHandover",
      ),
  ...
  const e9Body = `# SIM-003 Blocked Path — Execution Note (E9) — OFFICIAL
...
`;
  ...
  const reportBody = `# Sprint 7B Phase 3A — SIM-003 Blocked Path (Official Execution Report)
...
`;
  ...
  recordStep("EXECUTION_REPORT: SIM-003.md written", "PASS", {
    file: path.relative(process.cwd(), REPORT_PATH),
  });

  console.log(`\nSIM-003 OFFICIAL PASS — evidence at ${path.relative(process.cwd(), EVIDENCE_DIR)}`);
  console.log(
    JSON.stringify(
      {
        scenario: "SIM-003",
...
main()
  .catch(async (err) => {
    console.error("\nSIM-003 FAIL — STOP ON FAIL");
    console.error(err);
    try {
      await mkdir(EVIDENCE_DIR, { recursive: true });
      await writeFile(
        path.join(EVIDENCE_DIR, "E9-execution-note.md"),
        `# SIM-003 Blocked Path — FAIL (STOP ON FAIL)
...
Execution halted per Stop-on-Fail rule. Do not proceed to SIM-005/006/007 until resolved.
...
`,
      ...
      await mkdir(path.resolve("docs/SPRINT_7B/PHASE3_SIM-003"), { recursive: true });
      await writeFile(
        path.resolve("docs/SPRINT_7B/PHASE3_SIM-003/FINAL_GREEN_CHECK.md"),
        `# SIM-003 — Final Green Check — **BLOCKED**
