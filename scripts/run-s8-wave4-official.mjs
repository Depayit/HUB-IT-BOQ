/**
 * Sprint 8 Wave 4 — full official execution (E0 + NP-011 + NP-010)
 *
 * Usage: node scripts/run-s8-wave4-official.mjs
 */
...
const E0_ROOT = path.join(WAVE4_ROOT, "evidence/E0-pre-run-baseline");

const SCENARIOS = ["NP-011", "NP-010"];
...
async function seedAndRun(npId) {
  console.log(`\n=== ${npId}: seed ===`);
  const seedOut = execSync(`node scripts/seed-s8-wave4-scenarios.mjs --scenario=${npId}`, {
    encoding: "utf8",
  });
  ...
  execSync(
    `npx tsx scripts/execute-s8-wave4-official.mjs --np=${npId.replace("NP-", "")} --project=${seed.projectId} --boq=${seed.boqVersionId}`,
    { stdio: "inherit" },
  );
...
main().catch((err) => {
  console.error("WAVE4 ORCHESTRATOR STOP:", err.message ?? err);
  process.exit(1);
});
