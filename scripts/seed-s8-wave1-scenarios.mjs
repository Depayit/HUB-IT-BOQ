/**
 * Sprint 8 Wave 1 scenario seeds — NP-001, NP-002, NP-008
 *
 * Usage:
 *   node scripts/seed-s8-wave1-scenarios.mjs --scenario=NP-001
 *   node scripts/seed-s8-wave1-scenarios.mjs --scenario=NP-002
 *   node scripts/seed-s8-wave1-scenarios.mjs --scenario=NP-008
 *
 * Fresh Project IDs + BOQ Version IDs per run. No Sprint 7 ID reuse.
 */
...
async function seedHappy(npId, title) {
  ...
  const projectDiscipline = await prisma.project_disciplines.create({
    data: {
      ...
      included_flag: true,
      scope_description: "Power distribution scope for co-worker simulation.",
      risk_level: "Medium",
    },
  });
  ...
  return {
    scenario: npId,
    seed_profile: npId === "NP-001" ? "duplicate-approval" : "wrong-role-approval",
...
async function seedMultiBlock() {
  const npId = "NP-008";
  const project = await prisma.projects.create({
    data: {
      project_name: "NP-008 Multiple BLOCK Causes Project",
      client_id: `${npId}-CLIENT`,
      project_type: "Datacenter",
      it_load_kw: new Prisma.Decimal(1500),
      rack_count: 100,
  ...
  await prisma.design_basis_versions.create({
    data: {
      ...
      redundancy_assumption: "2N power, N+1 cooling",
      technical_compliance_basis: "Uptime Institute Tier III",
      customer_requirement_reference: "RFP-NP-008",
  ...
  const projectDiscipline = await prisma.project_disciplines.create({
    data: {
      ...
      discipline_id: pwrDiscipline.discipline_id,
      included_flag: true,
      scope_description: "Power scope — multi-block validation drill.",
      risk_level: "Medium",
    },
  });
  ...
  return {
    ...
    expected_block_rules: ["DESIGN_BASIS_NOT_APPROVED", "DOC_TOR_REQUIRED"],
    projectId: project.project_id,
    boqVersionId: boqVersion.boq_version_id,
    projectDisciplineId: projectDiscipline.project_discipline_id,
    documentIds: docs.map((d) => d.document_id),
...
async function main() {
  ...
  const scenario = args.scenario ?? "NP-002";
  const allowed = ["NP-001", "NP-002", "NP-008", "seed-np-001", "seed-np-002", "seed-np-008"];
  const normalized = scenario.replace(/^seed-/, "").toUpperCase();
  if (!allowed.includes(scenario) && !allowed.includes(normalized)) {
    throw new Error(`Unsupported scenario ${scenario}. Use NP-001, NP-002, or NP-008.`);
  }
  const npId =
    normalized === "NP-001" || scenario === "seed-np-001"
      ? "NP-001"
      : normalized === "NP-008" || scenario === "seed-np-008"
        ? "NP-008"
        : "NP-002";
  ...
  const result =
    npId === "NP-008"
      ? await seedMultiBlock()
      : npId === "NP-001"
        ? await seedHappy(npId, "NP-001 Duplicate Approval Project")
        : await seedHappy(npId, "NP-002 Wrong Role Approval Project");
