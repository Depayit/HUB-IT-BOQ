/**
 * Sprint 8 Wave 3 scenario seeds — NP-009, NP-012
 *
 * Usage:
 *   node scripts/seed-s8-wave3-scenarios.mjs --scenario=NP-009
 */
...
async function seedNp009() {
  return {
    ...(await createProjectBase(
      "NP-009",
      "NP-009 Stale Validation Snapshot Project",
      "Power distribution scope — stale validation drill.",
    )),
    seed_profile: "stale-validation-snapshot",
    primary_personas: ["Engineer", "Admin/Ops"],
  };
}

async function seedNp012() {
  return {
    ...(await createProjectBase(
      "NP-012",
      "NP-012 Cross-user Workflow Conflict Project",
      "Power distribution scope — concurrency drill.",
    )),
    seed_profile: "cross-user-workflow-conflict",
    primary_personas: ["Engineer", "Manager"],
    secondary_personas: ["Procurement"],
  };
}
...
async function main() {
  ...
  const scenario = (args.scenario ?? "NP-009").replace(/^seed-/, "").toUpperCase();
  if (!SEEDERS[scenario]) {
    throw new Error(`Unsupported scenario ${scenario}. Use NP-009 or NP-012.`);
  }
