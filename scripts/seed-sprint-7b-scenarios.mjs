 ...
 * - SIM-001 only at this point (other SIMs are out of scope for Phase 1).
...
async function main() {
  ...
  if (scenario !== "SIM-001") {
    throw new Error(
      `Only SIM-001 is supported in Sprint 7B Phase 1 (got ${scenario})`,
    );
  }

  await ensureMasters();
  const result = await seedSim001();
