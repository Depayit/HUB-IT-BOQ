...
/**
 * SIM-003 Blocked Path seed (blocked-core profile).
 *
 * Per scenario-seed-manifest.json:
 *   { scenario_type: "Blocked Path", seed_profile: "blocked-core",
 *     design_basis: "Draft", omit_documents: ["TOR"],
 *     expected_rules: ["DESIGN_BASIS_NOT_APPROVED","DOC_TOR_REQUIRED"],
 *     expected_readiness: "Blocked" }
 */
async function seedSim003() {
  const project = await prisma.projects.create({
    data: {
      project_name: "SIM-003 Blocked Path Project",
      client_id: "SIM-003-CLIENT",
      project_type: "Datacenter",
      it_load_kw: new Prisma.Decimal(1500),
      rack_count: 100,
      rack_density_kw_per_rack: new Prisma.Decimal(15),
  ...
  await prisma.design_basis_versions.create({
    data: {
      ...
      redundancy_assumption: "2N power, N+1 cooling",
      technical_compliance_basis: "Uptime Institute Tier III",
      customer_requirement_reference: "RFP-2026-003",
